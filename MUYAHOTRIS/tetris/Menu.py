...
from Tetris import *
import time
from Variable import Menu

class Menu:

    def __init__(self):
        pygame.init()
        Menu.infoObject = pygame.display.Info()
        self.tetris=Tetris()
        #self.database = Database()
        self.w = Menu.menu_display_w
        self.h = Menu.menu_display_h
        self.Mode = Menu.initial_mode
        #self.id=Var.initial_id
        #self.score=Var.initial_score
        self.page=Menu.initial_page
        self.surface=pygame.display.set_mode((self.w,self.h),RESIZABLE)
        self.mytheme=Menu.mytheme
        self.mytheme2=Menu.mytheme_help
        self.menu = pygame_menu.Menu(self.h,self.w, '', theme=self.mytheme)
        self.font_main=Menu.font_main   # 메인 폰트 사이즈
        self.font_sub=Menu.font_sub     # 서브 폰트 사이즈
        self.widget_margin_main=Menu.widget_margin_main         #메인 위젯 사이 간격
        self.widget_margin_showpage=Menu.widget_margin_showpage #show 페이지 위젯 사이 간격
        self.widget_margin_rank=Menu.widget_margin_rank         #rank 페이지 위젯 사이 간격
        self.margin_main=Menu.margin_main                       #메인 페이지 x,y 위젯 시작 위치
        self.margin_show=Menu.margin_show                       #show 페이지 x,y 위젯 시작 위치
        self.margin_help=Menu.margin_help                       #help 페이지 back 위치
        self.margin_rank=Menu.margin_rank                       #rank 페이지 x,y 위젯 시작 위치
   
    def run(self):   # 실행하는 함
        print('test2')
        self.page=Menu.initial_page   #시작하면 기본 모드로 모드가 설정
        self.menu.clear()
    ...
    def help(self): # help 페이
        self.page='page7'
        self.surface = pygame.display.set_mode(Menu.help_screen)
        self.menu = pygame_menu.Menu(Menu.help_h, Menu.help_w, '', theme=self.mytheme2)
    ...
    def show_game(self):
        ...
        self.menu.add_button('         back         ', self.rest, font_size=self.font_main)
